...
const totalSum = (input) => {
  return input.reduce((accumulator, currentValue) => {
    const runningTotal = currentValue.reduce((innerAccumulator, innerCurrentValue) => {
      return innerAccumulator + innerCurrentValue
    }, 0);
    return runningTotal+accumulator
...
const divisibleByFiveTwoToThePower = (input) => {
  let numbers = input.filter((n) => {
    return !(n === isNaN)

  });
  console.log(numbers)
  let fives = numbers.filter((n) => {
    return !(n%5 === 0)
  });
  console.log(numbers)
  let final = fives.map( n => n^2)
  console.log(final)
  return final
};
divisibleByFiveTwoToThePower([ [0,2,5, 'k' ,4], [2,4,10], [] ])
//currently returning 2 for each array within the larger array. Need to refactor to fix.
/* ------------------------------------------------------------------------------------------------
CHALLENGE 4
...
let findMaleAndFemale = (data) => {
  ...
  data.filter((person => {
    if(person.gender === 'female' || person.gender === 'male'){
      genderedArray.push(person.name)
    }
    return genderedArray
  }))
  return genderedArray.join(' and ')
};
...
//reduce to return final
  ...
  data.reduce( characters => {
    data.map(characters => {
      ...
      if(characters[i].height >= characters[b].height){
